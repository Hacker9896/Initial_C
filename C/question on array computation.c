...
should calculate the total cost of these purchases, then display it with 6 decimal places.
...
int main(){
	...
	for ( i = 0; i < item ; i ++){
		scanf("%lf", &weight_required[i]);
		if ( price < weight_required[i]*array[i]){
			price =  price + weight_required[i]*array[i];
		}
